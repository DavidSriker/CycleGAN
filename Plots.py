...
def plotLosses(g_loss, d_loss, epochs, name):
    ...
    plt.grid()
    fig.savefig('cycle_{:}_G_loss_vs_Iteration.png'.format(name),
                format='png',
                dpi=100,
                bbox_inches='tight')
    ...
    plt.grid()
    fig.savefig('cycle_{:}_D_loss_vs_Iteration.png'.format(name),
                format='png',
                dpi=100,
                bbox_inches='tight')
    ...
    plt.grid()
    fig.savefig('cycle_{:}_Losses_vs_Epochs.png'.format(name),
                format='png',
                dpi=100,
                bbox_inches='tight')
    return
...
def plotValidationAccuracy(mean_iou, mean_dice, epochs, name):
    ...
    plt.grid()
    fig.savefig('cycle_{:}_Scores_vs_Epochs.png'.format(name),
                format='png',
                dpi=100,
                bbox_inches='tight')
